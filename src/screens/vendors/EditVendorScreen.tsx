...
import { LocationService, Country, State } from "@/api_service/location";
import { 
  Building2, 
  MapPin, 
  Landmark, 
...
export default function EditVendorScreen() {
  ...
  // We maintain a map of country_id to states for dynamic rendering of multiple addresses
  const [dbStatesMap, setDbStatesMap] = useState<Record<number, State[]>>({});
  ...
  useEffect(() => {
    addresses.forEach(addr => {
      if (addr.country_id && !dbStatesMap[addr.country_id]) {
        LocationService.getStatesByCountry(addr.country_id)
          .then(states => {
            setDbStatesMap(prev => ({ ...prev, [addr.country_id]: states }));
          })
          .catch(console.error);
      }
    });
  }, [addresses, dbStatesMap]);
  ...
  const addAddress = () => {
    setAddresses(prev => [...prev, {
      ...
      city: "",
      pincode: "",
      country_id: dbCountries.length > 0 ? dbCountries[0].id : 0
    }]);
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select required value={addr.country_id || ""} onChange={(e) => updateAddress(idx, 'country_id', parseInt(e.target.value))} className="mt-1 block w-full rounded-lg border-0 py-2 px-3 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm bg-white">
                      {addr.country_id === 0 && <option value="" disabled>Select Country</option>}
                  ...
                  <div>
                    ...
                    <select value={addr.state_id || ""} onChange={(e) => updateAddress(idx, 'state_id', parseInt(e.target.value) || undefined)} className="mt-1 block w-full rounded-lg border-0 py-2 px-3 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm bg-white">
                      <option value="">Select State</option>
                      {dbStatesMap[addr.country_id]?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                  </div>
                  <div>
